// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "contact": {
            "name": "Dashboard Team",
            "email": "support@example.com"
        },
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/ai/sessions": {
            "post": {
                "description": "Create a new AI session for chart creation or editing. Creates a chart draft.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "ai"
                ],
                "summary": "Create a new AI session",
                "parameters": [
                    {
                        "description": "Session creation request",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.CreateAISessionRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.AISessionResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/ai/sessions/{id}": {
            "get": {
                "description": "Get an AI session by ID with current chart state",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "ai"
                ],
                "summary": "Get AI session",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Session ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.AISessionResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "delete": {
                "description": "Cancel the AI session and discard the draft",
                "tags": [
                    "ai"
                ],
                "summary": "Cancel AI session",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Session ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/ai/sessions/{id}/events": {
            "get": {
                "description": "Subscribe to real-time updates for an AI session via Server-Sent Events",
                "produces": [
                    "text/event-stream"
                ],
                "tags": [
                    "ai"
                ],
                "summary": "Subscribe to AI session events",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Session ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "SSE stream",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/ai/sessions/{id}/messages": {
            "post": {
                "description": "Send a user message to an AI session",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "ai"
                ],
                "summary": "Send message to AI session",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Session ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Message content",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.SendMessageRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.AIMessage"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/ai/sessions/{id}/save": {
            "post": {
                "description": "Save the AI session by publishing the draft as a new final version",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "ai"
                ],
                "summary": "Save AI session (publish draft)",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Session ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Chart"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/charts": {
            "get": {
                "description": "Get a paginated list of charts (latest version of each) with optional filtering",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "charts"
                ],
                "summary": "List charts",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Filter by name (partial match)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filter by chart type",
                        "name": "chart_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filter by data source ID",
                        "name": "datasource_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filter by tag",
                        "name": "tag",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 1,
                        "description": "Page number",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "Page size",
                        "name": "page_size",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ChartListResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "description": "Create a new chart with data source binding and visualization config. Creates as version 1 with status \"final\".",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "charts"
                ],
                "summary": "Create a new chart",
                "parameters": [
                    {
                        "description": "Chart data",
                        "name": "chart",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.CreateChartRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Chart"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/charts/summaries": {
            "get": {
                "description": "Get lightweight chart summaries for card-based selection UI",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "charts"
                ],
                "summary": "Get chart summaries",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 50,
                        "description": "Maximum number of summaries",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ChartSummary"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/charts/{id}": {
            "get": {
                "description": "Get the latest version of a chart by ID",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "charts"
                ],
                "summary": "Get a chart",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Chart ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Chart"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "put": {
                "description": "Update the latest version of a chart in-place (for manual edits)",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "charts"
                ],
                "summary": "Update a chart",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Chart ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Chart update data",
                        "name": "chart",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.UpdateChartRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Chart"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete all versions of a chart by ID",
                "tags": [
                    "charts"
                ],
                "summary": "Delete a chart",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Chart ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/charts/{id}/draft": {
            "get": {
                "description": "Get the draft version of a chart (if exists)",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "charts"
                ],
                "summary": "Get chart draft",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Chart ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Chart"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete the draft version of a chart (if exists)",
                "tags": [
                    "charts"
                ],
                "summary": "Delete chart draft",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Chart ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/charts/{id}/version-info": {
            "get": {
                "description": "Get version metadata for a chart (version count, has draft, etc.)",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "charts"
                ],
                "summary": "Get chart version info",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Chart ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ChartVersionInfo"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/charts/{id}/versions": {
            "get": {
                "description": "Get all versions of a chart by ID",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "charts"
                ],
                "summary": "List chart versions",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Chart ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/charts/{id}/versions/{version}": {
            "get": {
                "description": "Get a specific version of a chart by ID and version number",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "charts"
                ],
                "summary": "Get a specific chart version",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Chart ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Version number",
                        "name": "version",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Chart"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete a specific version of a chart",
                "tags": [
                    "charts"
                ],
                "summary": "Delete a chart version",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Chart ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Version number",
                        "name": "version",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/components": {
            "get": {
                "description": "Get a paginated list of components with optional filtering",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "components"
                ],
                "summary": "List components",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Filter by system",
                        "name": "system",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filter by source",
                        "name": "source",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filter by category",
                        "name": "category",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filter by tag",
                        "name": "tag",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 1,
                        "description": "Page number",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "Page size",
                        "name": "page_size",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ComponentListResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "description": "Create a new dashboard component or chart",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "components"
                ],
                "summary": "Create a new component",
                "parameters": [
                    {
                        "description": "Component data",
                        "name": "component",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.CreateComponentRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Component"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/components/systems": {
            "get": {
                "description": "Get all component systems and sources with counts",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "components"
                ],
                "summary": "Get component systems",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ComponentSystemsResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/components/{id}": {
            "get": {
                "description": "Get a component by ID",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "components"
                ],
                "summary": "Get a component",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Component ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Component"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "put": {
                "description": "Update an existing component",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "components"
                ],
                "summary": "Update a component",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Component ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Component update data",
                        "name": "component",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.UpdateComponentRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Component"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete a component by ID",
                "tags": [
                    "components"
                ],
                "summary": "Delete a component",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Component ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/dashboards": {
            "get": {
                "description": "Get a paginated list of dashboards with optional filtering",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "dashboards"
                ],
                "summary": "List dashboards",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Filter by name (partial match)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "boolean",
                        "description": "Filter by public status",
                        "name": "is_public",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 1,
                        "description": "Page number",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "Page size",
                        "name": "page_size",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DashboardListResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "description": "Create a new dashboard with panels and embedded charts",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "dashboards"
                ],
                "summary": "Create a new dashboard",
                "parameters": [
                    {
                        "description": "Dashboard data",
                        "name": "dashboard",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.CreateDashboardRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Dashboard"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/dashboards/{id}": {
            "get": {
                "description": "Get a dashboard by ID (includes panels and charts)",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "dashboards"
                ],
                "summary": "Get a dashboard",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Dashboard ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Dashboard"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "put": {
                "description": "Update an existing dashboard",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "dashboards"
                ],
                "summary": "Update a dashboard",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Dashboard ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dashboard update data",
                        "name": "dashboard",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.UpdateDashboardRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Dashboard"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete a dashboard by ID",
                "tags": [
                    "dashboards"
                ],
                "summary": "Delete a dashboard",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Dashboard ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/datasources": {
            "get": {
                "description": "Retrieve all datasources with pagination and optional type filter",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "datasources"
                ],
                "summary": "List all datasources",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "Number of items per page",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Number of items to skip",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filter by datasource type (api, websocket, file)",
                        "name": "type",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "description": "Create a new data source (API, WebSocket, or File)",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "datasources"
                ],
                "summary": "Create a new datasource",
                "parameters": [
                    {
                        "description": "Datasource to create",
                        "name": "datasource",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.CreateDatasourceRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Datasource"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/datasources/test": {
            "post": {
                "description": "Test a datasource connection without saving it",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "datasources"
                ],
                "summary": "Test a datasource connection",
                "parameters": [
                    {
                        "description": "Datasource configuration to test",
                        "name": "datasource",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.TestDatasourceRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.TestDatasourceResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/datasources/{id}": {
            "get": {
                "description": "Retrieve a single datasource by its ID",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "datasources"
                ],
                "summary": "Get a datasource by ID",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Datasource ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Datasource"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "put": {
                "description": "Update an existing datasource by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "datasources"
                ],
                "summary": "Update a datasource",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Datasource ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Datasource updates",
                        "name": "datasource",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.UpdateDatasourceRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Datasource"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete a datasource by ID",
                "tags": [
                    "datasources"
                ],
                "summary": "Delete a datasource",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Datasource ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/datasources/{id}/health": {
            "post": {
                "description": "Check the health of a specific datasource and update its status",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "datasources"
                ],
                "summary": "Check datasource health",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Datasource ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.HealthInfo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/datasources/{id}/query": {
            "post": {
                "description": "Execute a query and return normalized results",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "datasources"
                ],
                "summary": "Execute a query against a datasource",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Datasource ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Query to execute",
                        "name": "query",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.QueryRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.QueryResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/datasources/{id}/schema": {
            "get": {
                "description": "Retrieve tables and columns for SQL datasources. Only SQL-type datasources support this endpoint.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "datasources"
                ],
                "summary": "Get database schema for a SQL datasource",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Datasource ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.SchemaResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/health": {
            "get": {
                "description": "Check if the server and dependencies are healthy",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "System"
                ],
                "summary": "Health check",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "503": {
                        "description": "Service Unavailable",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/layouts": {
            "get": {
                "description": "Retrieve all layouts with pagination",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "layouts"
                ],
                "summary": "List all layouts",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "Number of items per page",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Number of items to skip",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "description": "Create a new dashboard layout with grid configuration and panels",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "layouts"
                ],
                "summary": "Create a new layout",
                "parameters": [
                    {
                        "description": "Layout to create",
                        "name": "layout",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.CreateLayoutRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Layout"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/layouts/{id}": {
            "get": {
                "description": "Retrieve a single layout by its ID",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "layouts"
                ],
                "summary": "Get a layout by ID",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Layout ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Layout"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "put": {
                "description": "Update an existing layout by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "layouts"
                ],
                "summary": "Update a layout",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Layout ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Layout updates",
                        "name": "layout",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.UpdateLayoutRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Layout"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete a layout by ID",
                "tags": [
                    "layouts"
                ],
                "summary": "Delete a layout",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Layout ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/mcp/message": {
            "post": {
                "description": "Process a JSON-RPC message for MCP protocol",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "MCP"
                ],
                "summary": "Handle MCP Message",
                "parameters": [
                    {
                        "description": "JSON-RPC request",
                        "name": "message",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_mcp.JSONRPCRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/internal_mcp.JSONRPCResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/internal_mcp.JSONRPCResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/internal_mcp.JSONRPCResponse"
                        }
                    }
                }
            }
        },
        "/mcp/sse": {
            "get": {
                "description": "Establish an SSE connection for MCP protocol",
                "produces": [
                    "text/event-stream"
                ],
                "tags": [
                    "MCP"
                ],
                "summary": "MCP SSE Connection",
                "responses": {
                    "200": {
                        "description": "SSE stream",
                        "schema": {
                            "type": "string"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "github_com_tviviano_dashboard_internal_models.AIMessage": {
            "description": "A message in the AI conversation",
            "type": "object",
            "properties": {
                "content": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "role": {
                    "description": "\"user\" | \"assistant\" | \"system\"",
                    "type": "string"
                },
                "timestamp": {
                    "type": "string"
                },
                "tool_calls": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ToolCall"
                    }
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.AISession": {
            "description": "AI session for chart creation/editing",
            "type": "object",
            "properties": {
                "chart_id": {
                    "description": "Chart UUID being edited",
                    "type": "string"
                },
                "chart_version": {
                    "description": "Version being edited (always a draft)",
                    "type": "integer"
                },
                "created": {
                    "type": "string"
                },
                "id": {
                    "description": "UUID",
                    "type": "string"
                },
                "messages": {
                    "description": "Conversation history",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.AIMessage"
                    }
                },
                "status": {
                    "description": "\"active\" | \"completed\" | \"cancelled\"",
                    "type": "string"
                },
                "updated": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.AISessionResponse": {
            "description": "Response containing AI session state",
            "type": "object",
            "properties": {
                "chart": {
                    "description": "Current chart state (draft)",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Chart"
                        }
                    ]
                },
                "session": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.AISession"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.APIConfig": {
            "type": "object",
            "required": [
                "method",
                "url"
            ],
            "properties": {
                "auth_credentials": {
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                },
                "auth_type": {
                    "description": "none, bearer, basic, api-key",
                    "type": "string"
                },
                "body": {
                    "type": "string"
                },
                "headers": {
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                },
                "method": {
                    "type": "string",
                    "enum": [
                        "GET",
                        "POST",
                        "PUT",
                        "DELETE",
                        "PATCH"
                    ]
                },
                "query_params": {
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                },
                "response_config": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.APIResponseConfig"
                },
                "retry_count": {
                    "type": "integer"
                },
                "retry_delay": {
                    "description": "milliseconds",
                    "type": "integer"
                },
                "timeout": {
                    "description": "seconds",
                    "type": "integer"
                },
                "url": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.APIResponseConfig": {
            "type": "object",
            "properties": {
                "data_path": {
                    "description": "DataPath is the JSON path to the array of records (e.g., \"data\", \"results\", \"items\")\nIf empty, assumes response is already an array or will be parsed as key-value pairs",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.CSVConfig": {
            "type": "object",
            "required": [
                "path"
            ],
            "properties": {
                "columns": {
                    "description": "explicit column names",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "delimiter": {
                    "description": "default: \",\"",
                    "type": "string"
                },
                "encoding": {
                    "description": "utf-8, ascii, etc.",
                    "type": "string"
                },
                "has_header": {
                    "description": "default: true",
                    "type": "boolean"
                },
                "path": {
                    "type": "string"
                },
                "watch_changes": {
                    "type": "boolean"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.Chart": {
            "description": "Chart with data source binding, query config, and visualization settings",
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "ai_session_id": {
                    "description": "Active AI session (drafts only)",
                    "type": "string"
                },
                "chart_type": {
                    "description": "bar, line, pie, gauge, etc.",
                    "type": "string"
                },
                "component_code": {
                    "description": "React component code",
                    "type": "string"
                },
                "created": {
                    "type": "string"
                },
                "data_mapping": {
                    "description": "How to map data to chart",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ChartDataMapping"
                        }
                    ]
                },
                "datasource_id": {
                    "description": "Reference to data source",
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "description": "UUID - same across versions",
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "options": {
                    "description": "ECharts options overrides",
                    "type": "object",
                    "additionalProperties": true
                },
                "query_config": {
                    "description": "How to query data",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ChartQueryConfig"
                        }
                    ]
                },
                "status": {
                    "description": "\"draft\" | \"final\"",
                    "type": "string"
                },
                "tags": {
                    "description": "Searchable tags",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "thumbnail": {
                    "description": "Base64 preview image for card display",
                    "type": "string"
                },
                "updated": {
                    "type": "string"
                },
                "use_custom_code": {
                    "description": "Whether custom code mode is enabled",
                    "type": "boolean"
                },
                "version": {
                    "description": "Version number (1, 2, 3...)",
                    "type": "integer"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ChartDataMapping": {
            "description": "Mapping configuration from data columns to chart axes/series",
            "type": "object",
            "properties": {
                "aggregation": {
                    "description": "Aggregation to apply (first, last, avg, etc.)",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DataAggregation"
                        }
                    ]
                },
                "filters": {
                    "description": "Client-side filters applied after data fetch",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DataFilter"
                    }
                },
                "group_by": {
                    "description": "Column to group/split series by",
                    "type": "string"
                },
                "label_col": {
                    "description": "Column for labels",
                    "type": "string"
                },
                "limit": {
                    "description": "Max rows to return",
                    "type": "integer"
                },
                "sort_by": {
                    "description": "Column to sort by",
                    "type": "string"
                },
                "sort_order": {
                    "description": "asc or desc",
                    "type": "string"
                },
                "x_axis": {
                    "description": "Column for X axis (categories)",
                    "type": "string"
                },
                "x_axis_format": {
                    "description": "Format for X axis values: chart, chart_time, chart_date, chart_datetime, short, long, etc.",
                    "type": "string"
                },
                "x_axis_label": {
                    "description": "Label for X axis (e.g., \"Time\", \"Date\")",
                    "type": "string"
                },
                "y_axis": {
                    "description": "Columns for Y axis (values/series)",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "y_axis_label": {
                    "description": "Label for Y axis (e.g., \"Temperature (°F)\", \"Count\")",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ChartListResponse": {
            "description": "Response containing a list of charts with pagination",
            "type": "object",
            "properties": {
                "charts": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Chart"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "page_size": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ChartQueryConfig": {
            "description": "Query configuration for fetching chart data",
            "type": "object",
            "properties": {
                "params": {
                    "description": "Query parameters",
                    "type": "object",
                    "additionalProperties": true
                },
                "raw": {
                    "description": "SQL query, filter, or API path",
                    "type": "string"
                },
                "type": {
                    "description": "sql, csv_filter, stream_filter, api",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ChartSummary": {
            "description": "Minimal chart info for selection cards and lists",
            "type": "object",
            "properties": {
                "chart_type": {
                    "type": "string"
                },
                "datasource_id": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "thumbnail": {
                    "type": "string"
                },
                "version": {
                    "type": "integer"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ChartVersionInfo": {
            "description": "Version info for a chart",
            "type": "object",
            "properties": {
                "has_draft": {
                    "description": "Whether a draft version exists",
                    "type": "boolean"
                },
                "id": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "version": {
                    "type": "integer"
                },
                "version_count": {
                    "description": "Total versions for this chart id",
                    "type": "integer"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ColumnInfo": {
            "type": "object",
            "properties": {
                "default": {
                    "description": "Default value if any",
                    "type": "string"
                },
                "name": {
                    "description": "Column name",
                    "type": "string"
                },
                "nullable": {
                    "description": "Whether column allows NULL",
                    "type": "boolean"
                },
                "primary_key": {
                    "description": "Whether column is part of primary key",
                    "type": "boolean"
                },
                "type": {
                    "description": "Data type (e.g., \"varchar\", \"integer\")",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.Component": {
            "description": "Component model for dashboard charts and widgets",
            "type": "object",
            "required": [
                "component_code",
                "name",
                "source",
                "system"
            ],
            "properties": {
                "component_code": {
                    "type": "string"
                },
                "created": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "metadata": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ComponentMetadata"
                },
                "name": {
                    "type": "string"
                },
                "source": {
                    "type": "string"
                },
                "system": {
                    "type": "string"
                },
                "updated": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ComponentListResponse": {
            "description": "Response containing a list of components with pagination",
            "type": "object",
            "properties": {
                "components": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Component"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "page_size": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ComponentMetadata": {
            "description": "Metadata for component categorization and configuration",
            "type": "object",
            "properties": {
                "category": {
                    "type": "string"
                },
                "custom": {
                    "type": "object",
                    "additionalProperties": true
                },
                "datasourceType": {
                    "type": "string"
                },
                "refreshInterval": {
                    "type": "integer"
                },
                "requiredApis": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "visualization": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.VisualizationConfig"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ComponentSource": {
            "description": "Source category containing component count",
            "type": "object",
            "properties": {
                "component_count": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ComponentSystem": {
            "description": "System category containing multiple sources",
            "type": "object",
            "properties": {
                "name": {
                    "type": "string"
                },
                "sources": {
                    "type": "object",
                    "additionalProperties": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ComponentSource"
                    }
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ComponentSystemsResponse": {
            "description": "Response containing component organization hierarchy",
            "type": "object",
            "properties": {
                "systems": {
                    "type": "object",
                    "additionalProperties": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ComponentSystem"
                    }
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.CreateAISessionRequest": {
            "description": "Request body for creating a new AI session",
            "type": "object",
            "properties": {
                "chart_id": {
                    "description": "Existing chart ID to edit (optional, omit for new chart)",
                    "type": "string"
                },
                "initial_message": {
                    "description": "First user message (optional)",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.CreateChartRequest": {
            "description": "Request body for creating a new chart",
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "chart_type": {
                    "type": "string"
                },
                "component_code": {
                    "type": "string"
                },
                "data_mapping": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ChartDataMapping"
                },
                "datasource_id": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "options": {
                    "type": "object",
                    "additionalProperties": true
                },
                "query_config": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ChartQueryConfig"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "thumbnail": {
                    "type": "string"
                },
                "use_custom_code": {
                    "type": "boolean"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.CreateComponentRequest": {
            "description": "Request body for creating a new component",
            "type": "object",
            "required": [
                "component_code",
                "name",
                "source",
                "system"
            ],
            "properties": {
                "component_code": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "metadata": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ComponentMetadata"
                },
                "name": {
                    "type": "string"
                },
                "source": {
                    "type": "string"
                },
                "system": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.CreateDashboardRequest": {
            "description": "Request body for creating a new dashboard",
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "description": {
                    "type": "string"
                },
                "metadata": {
                    "type": "object",
                    "additionalProperties": true
                },
                "name": {
                    "type": "string"
                },
                "panels": {
                    "description": "Panels with optional chart_id",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DashboardPanel"
                    }
                },
                "settings": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DashboardSettings"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.CreateDatasourceRequest": {
            "type": "object",
            "required": [
                "config",
                "name",
                "type"
            ],
            "properties": {
                "config": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DatasourceConfig"
                },
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "type": {
                    "enum": [
                        "sql",
                        "csv",
                        "socket",
                        "api"
                    ],
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DatasourceType"
                        }
                    ]
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.CreateLayoutRequest": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "description": {
                    "type": "string"
                },
                "grid": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.GridConfig"
                },
                "name": {
                    "type": "string"
                },
                "panels": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Panel"
                    }
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.Dashboard": {
            "description": "Dashboard with panels that reference standalone charts",
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "created": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "metadata": {
                    "type": "object",
                    "additionalProperties": true
                },
                "name": {
                    "type": "string"
                },
                "panels": {
                    "description": "Panels with chart_id references",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DashboardPanel"
                    }
                },
                "settings": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DashboardSettings"
                },
                "updated": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.DashboardListResponse": {
            "description": "Response containing a list of dashboards with pagination",
            "type": "object",
            "properties": {
                "dashboards": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Dashboard"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "page_size": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.DashboardPanel": {
            "description": "Panel position and size in the grid with optional chart reference",
            "type": "object",
            "properties": {
                "chart_id": {
                    "description": "Reference to chart",
                    "type": "string"
                },
                "h": {
                    "type": "integer"
                },
                "id": {
                    "type": "string"
                },
                "w": {
                    "type": "integer"
                },
                "x": {
                    "type": "integer"
                },
                "y": {
                    "type": "integer"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.DashboardSettings": {
            "description": "Dashboard settings and preferences",
            "type": "object",
            "properties": {
                "allow_export": {
                    "type": "boolean"
                },
                "default_view": {
                    "type": "string"
                },
                "is_public": {
                    "type": "boolean"
                },
                "refresh_interval": {
                    "type": "integer"
                },
                "theme": {
                    "type": "string"
                },
                "timezone": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.DataAggregation": {
            "description": "Aggregation configuration for data transformation",
            "type": "object",
            "properties": {
                "count": {
                    "description": "Row count (for limit)",
                    "type": "integer"
                },
                "field": {
                    "description": "Column to aggregate (for min/max/avg/sum)",
                    "type": "string"
                },
                "sort_by": {
                    "description": "Column to sort by (for first/last)",
                    "type": "string"
                },
                "type": {
                    "description": "first, last, min, max, avg, sum, count, limit",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.DataFilter": {
            "description": "Filter condition for data transformation",
            "type": "object",
            "properties": {
                "field": {
                    "description": "Column name to filter on",
                    "type": "string"
                },
                "op": {
                    "description": "Operator: eq, neq, gt, gte, lt, lte, contains, in, notIn, isNull, isNotNull",
                    "type": "string"
                },
                "value": {
                    "description": "Value to compare against (can be array for 'in' operator)"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.Datasource": {
            "type": "object",
            "required": [
                "config",
                "name",
                "type"
            ],
            "properties": {
                "config": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DatasourceConfig"
                },
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "health": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.HealthInfo"
                },
                "id": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "type": {
                    "enum": [
                        "sql",
                        "csv",
                        "socket",
                        "api"
                    ],
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DatasourceType"
                        }
                    ]
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.DatasourceConfig": {
            "type": "object",
            "properties": {
                "api": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.APIConfig"
                },
                "csv": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.CSVConfig"
                },
                "socket": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.SocketConfig"
                },
                "sql": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.SQLConfig"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.DatasourceType": {
            "type": "string",
            "enum": [
                "sql",
                "csv",
                "socket",
                "api"
            ],
            "x-enum-varnames": [
                "DatasourceTypeSQL",
                "DatasourceTypeCSV",
                "DatasourceTypeSocket",
                "DatasourceTypeAPI"
            ]
        },
        "github_com_tviviano_dashboard_internal_models.GridConfig": {
            "type": "object",
            "properties": {
                "columns": {
                    "description": "Number of columns (e.g., 12)",
                    "type": "integer"
                },
                "compact_type": {
                    "description": "\"vertical\", \"horizontal\", or null",
                    "type": "string"
                },
                "grid_unit": {
                    "description": "Base spacing unit (32px = $spacing-08)",
                    "type": "integer"
                },
                "max_rows": {
                    "description": "Maximum number of rows",
                    "type": "integer"
                },
                "row_height": {
                    "description": "Height of each row in pixels",
                    "type": "integer"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.HealthInfo": {
            "type": "object",
            "properties": {
                "error_message": {
                    "type": "string"
                },
                "last_check": {
                    "type": "string"
                },
                "last_success": {
                    "type": "string"
                },
                "response_time": {
                    "description": "milliseconds",
                    "type": "integer"
                },
                "status": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.HealthStatus"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.HealthStatus": {
            "type": "string",
            "enum": [
                "unknown",
                "healthy",
                "unhealthy",
                "degraded"
            ],
            "x-enum-varnames": [
                "HealthStatusUnknown",
                "HealthStatusHealthy",
                "HealthStatusUnhealthy",
                "HealthStatusDegraded"
            ]
        },
        "github_com_tviviano_dashboard_internal_models.Layout": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "grid": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.GridConfig"
                },
                "id": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "panels": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Panel"
                    }
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.Panel": {
            "type": "object",
            "required": [
                "h",
                "id",
                "w",
                "x",
                "y"
            ],
            "properties": {
                "content": {
                    "description": "Panel content reference",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.PanelContent"
                        }
                    ]
                },
                "h": {
                    "description": "Height in rows",
                    "type": "integer",
                    "minimum": 1
                },
                "id": {
                    "description": "Unique panel identifier",
                    "type": "string"
                },
                "max_h": {
                    "description": "Maximum height",
                    "type": "integer"
                },
                "max_w": {
                    "description": "Maximum width",
                    "type": "integer"
                },
                "min_h": {
                    "description": "Minimum height",
                    "type": "integer"
                },
                "min_w": {
                    "description": "Minimum width",
                    "type": "integer"
                },
                "static": {
                    "description": "Cannot be moved/resized",
                    "type": "boolean"
                },
                "w": {
                    "description": "Width in columns",
                    "type": "integer",
                    "minimum": 1
                },
                "x": {
                    "description": "Column position (0-indexed)",
                    "type": "integer"
                },
                "y": {
                    "description": "Row position (0-indexed)",
                    "type": "integer"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.PanelContent": {
            "type": "object",
            "properties": {
                "chart_id": {
                    "description": "Reference to chart component",
                    "type": "string"
                },
                "component_id": {
                    "description": "Reference to custom component",
                    "type": "string"
                },
                "config": {
                    "description": "Panel-specific configuration",
                    "type": "object",
                    "additionalProperties": true
                },
                "type": {
                    "description": "\"chart\", \"text\", \"image\", \"custom\"",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.Query": {
            "type": "object",
            "properties": {
                "params": {
                    "description": "Query parameters",
                    "type": "object",
                    "additionalProperties": true
                },
                "raw": {
                    "description": "Raw query string (SQL, filter expression, etc.)",
                    "type": "string"
                },
                "type": {
                    "description": "Query type",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.QueryType"
                        }
                    ]
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.QueryRequest": {
            "type": "object",
            "required": [
                "query"
            ],
            "properties": {
                "query": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Query"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.QueryResponse": {
            "type": "object",
            "properties": {
                "duration": {
                    "description": "milliseconds",
                    "type": "integer"
                },
                "error": {
                    "type": "string"
                },
                "result_set": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ResultSet"
                },
                "success": {
                    "type": "boolean"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.QueryType": {
            "type": "string",
            "enum": [
                "sql",
                "csv_filter",
                "stream_filter",
                "api"
            ],
            "x-enum-varnames": [
                "QueryTypeSQL",
                "QueryTypeCSVFilter",
                "QueryTypeStreamFilter",
                "QueryTypeAPI"
            ]
        },
        "github_com_tviviano_dashboard_internal_models.ResultSet": {
            "type": "object",
            "properties": {
                "columns": {
                    "description": "Column names",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "metadata": {
                    "description": "Additional metadata",
                    "type": "object",
                    "additionalProperties": true
                },
                "rows": {
                    "description": "Data rows",
                    "type": "array",
                    "items": {
                        "type": "array",
                        "items": {}
                    }
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.SQLConfig": {
            "type": "object",
            "required": [
                "connection_string",
                "driver"
            ],
            "properties": {
                "connection_string": {
                    "type": "string"
                },
                "database": {
                    "type": "string"
                },
                "driver": {
                    "type": "string",
                    "enum": [
                        "postgres",
                        "mysql",
                        "sqlite",
                        "mssql",
                        "oracle"
                    ]
                },
                "host": {
                    "type": "string"
                },
                "max_connections": {
                    "type": "integer"
                },
                "options": {
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                },
                "password": {
                    "type": "string"
                },
                "port": {
                    "type": "integer"
                },
                "ssl": {
                    "type": "boolean"
                },
                "timeout": {
                    "description": "seconds",
                    "type": "integer"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.SchemaInfo": {
            "type": "object",
            "properties": {
                "database": {
                    "description": "Current database name",
                    "type": "string"
                },
                "tables": {
                    "description": "Tables in the database",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.TableInfo"
                    }
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.SchemaResponse": {
            "type": "object",
            "properties": {
                "duration": {
                    "description": "milliseconds",
                    "type": "integer"
                },
                "error": {
                    "type": "string"
                },
                "schema": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.SchemaInfo"
                },
                "success": {
                    "type": "boolean"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.SendMessageRequest": {
            "description": "Request body for sending a user message",
            "type": "object",
            "required": [
                "content"
            ],
            "properties": {
                "content": {
                    "description": "User message content",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.SocketConfig": {
            "type": "object",
            "required": [
                "protocol",
                "url"
            ],
            "properties": {
                "buffer_size": {
                    "description": "number of messages to buffer",
                    "type": "integer"
                },
                "headers": {
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                },
                "message_format": {
                    "description": "json, text, binary",
                    "type": "string"
                },
                "parser": {
                    "description": "parser configuration",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.SocketParserConfig"
                        }
                    ]
                },
                "ping_interval": {
                    "description": "seconds",
                    "type": "integer"
                },
                "protocol": {
                    "type": "string",
                    "enum": [
                        "tcp",
                        "udp",
                        "websocket"
                    ]
                },
                "reconnect_delay": {
                    "description": "milliseconds",
                    "type": "integer"
                },
                "reconnect_on_error": {
                    "type": "boolean"
                },
                "url": {
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.SocketParserConfig": {
            "type": "object",
            "properties": {
                "data_path": {
                    "description": "DataPath is the JSON path to the data payload (e.g., \"data\", \"payload\", \"message.readings\")\nSupports dot notation for nested paths. If empty, treats entire message as the data object.",
                    "type": "string"
                },
                "exclude_fields": {
                    "description": "ExcludeFields removes these fields from output",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "field_mappings": {
                    "description": "FieldMappings renames fields in the output (e.g., {\"temp\": \"temperature\", \"ts\": \"timestamp\"})",
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                },
                "include_fields": {
                    "description": "IncludeFields limits output to only these fields (empty = include all)",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "timestamp_field": {
                    "description": "TimestampField specifies which field contains the timestamp (default: use server receive time)",
                    "type": "string"
                },
                "timestamp_format": {
                    "description": "TimestampFormat is the Go time format string for parsing timestamps (default: RFC3339)\nCommon formats: \"2006-01-02T15:04:05Z07:00\" (RFC3339), \"2006-01-02 15:04:05\", unix timestamp",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.TableInfo": {
            "type": "object",
            "properties": {
                "columns": {
                    "description": "Columns in the table",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ColumnInfo"
                    }
                },
                "name": {
                    "description": "Table name",
                    "type": "string"
                },
                "schema": {
                    "description": "Schema/namespace (e.g., \"public\" for PostgreSQL)",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.TestDatasourceRequest": {
            "type": "object",
            "required": [
                "config",
                "type"
            ],
            "properties": {
                "config": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DatasourceConfig"
                },
                "type": {
                    "enum": [
                        "sql",
                        "csv",
                        "socket",
                        "api"
                    ],
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DatasourceType"
                        }
                    ]
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.TestDatasourceResponse": {
            "type": "object",
            "properties": {
                "data": {},
                "message": {
                    "type": "string"
                },
                "response_time": {
                    "description": "milliseconds",
                    "type": "integer"
                },
                "status": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.HealthStatus"
                },
                "success": {
                    "type": "boolean"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.ToolCall": {
            "description": "A tool call made by the AI",
            "type": "object",
            "properties": {
                "id": {
                    "type": "string"
                },
                "input": {
                    "description": "JSON string of tool input",
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "output": {
                    "description": "JSON string of tool output",
                    "type": "string"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.UpdateChartRequest": {
            "description": "Request body for updating an existing chart",
            "type": "object",
            "properties": {
                "chart_type": {
                    "type": "string"
                },
                "component_code": {
                    "type": "string"
                },
                "data_mapping": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ChartDataMapping"
                },
                "datasource_id": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "options": {
                    "type": "object",
                    "additionalProperties": true
                },
                "query_config": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ChartQueryConfig"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "thumbnail": {
                    "type": "string"
                },
                "use_custom_code": {
                    "type": "boolean"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.UpdateComponentRequest": {
            "description": "Request body for updating an existing component",
            "type": "object",
            "properties": {
                "component_code": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "metadata": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.ComponentMetadata"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.UpdateDashboardRequest": {
            "description": "Request body for updating an existing dashboard",
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "metadata": {
                    "type": "object",
                    "additionalProperties": true
                },
                "name": {
                    "type": "string"
                },
                "panels": {
                    "description": "Panels with optional chart_id",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DashboardPanel"
                    }
                },
                "settings": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DashboardSettings"
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.UpdateDatasourceRequest": {
            "type": "object",
            "properties": {
                "config": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.DatasourceConfig"
                },
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.UpdateLayoutRequest": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "grid": {
                    "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.GridConfig"
                },
                "name": {
                    "type": "string"
                },
                "panels": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_tviviano_dashboard_internal_models.Panel"
                    }
                }
            }
        },
        "github_com_tviviano_dashboard_internal_models.VisualizationConfig": {
            "description": "Configuration for visualization components",
            "type": "object",
            "properties": {
                "library": {
                    "description": "echarts, d3, custom",
                    "type": "string"
                },
                "type": {
                    "description": "bar, line, pie, gauge, etc.",
                    "type": "string"
                }
            }
        },
        "internal_mcp.JSONRPCError": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer"
                },
                "data": {},
                "message": {
                    "type": "string"
                }
            }
        },
        "internal_mcp.JSONRPCRequest": {
            "type": "object",
            "properties": {
                "id": {},
                "jsonrpc": {
                    "type": "string"
                },
                "method": {
                    "type": "string"
                },
                "params": {
                    "type": "object",
                    "additionalProperties": true
                }
            }
        },
        "internal_mcp.JSONRPCResponse": {
            "type": "object",
            "properties": {
                "error": {
                    "$ref": "#/definitions/internal_mcp.JSONRPCError"
                },
                "id": {},
                "jsonrpc": {
                    "type": "string"
                },
                "result": {}
            }
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "1.0",
	Host:             "localhost:3001",
	BasePath:         "/api",
	Schemes:          []string{},
	Title:            "GiVi-Solution Dashboard API",
	Description:      "Dashboard system with AI-powered chart generation",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
